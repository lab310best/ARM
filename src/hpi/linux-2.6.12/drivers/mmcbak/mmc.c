...
static u32 mmc_select_voltage(struct mmc_host *host, u32 ocr)
{
	...
	if (bit) {
		...
		ocr = 3 << bit;
		//modified by hzh, bit -> mmc_ocr_bit_to_vdd[bit]
...
static void mmc_decode_cid(struct mmc_cid *cid, u32 *resp, int is_sd)
{
	...
	if( is_sd ) {
		...
		cid->prod_name[6] = 0;
		cid->prod_name[7] = 0;
		cid->manfid	  = (resp[0] >> 8) & 0xff;
		cid->hwrev	  = resp[2] >> 24;
		cid->fwrev	  = 0;
		cid->serial = ((resp[2] >> 16) & 0xff) | (resp[2] & 0xff00) |
		               ((resp[2] & 0xff )<< 16) | (resp[3] & 0xff000000);
		cid->month  = ((resp[3] >> 8) & 0xf);
		cid->year   = (resp[3] >> 12) & 0xff;
	} else {
	...
	printk("cid: manfid: 0x%02x\n",cid->manfid);
	printk("       name: %s\n",cid->prod_name);
	printk("      hwrev: 0x%02x\n",cid->hwrev);
	printk("     serial: 0x%08x\n",cid->serial);
	printk("  prod date: %d/%d\n",cid->month,cid->year + 2000);
}
...
static void mmc_power_up(struct mmc_host *host)
{
	...
	host->ios.vdd = mmc_ocr_bit_to_vdd[bit];	//modified by hzh, bit -> mmc_ocr...
...
static int mmc_send_op_cond(struct mmc_host *host, u32 ocr, u32 *rocr, int *is_sd)
{
	struct mmc_command cmd,app_cmd;
	int i, err = 0;
	...
	*is_sd = 0;
	...
	app_cmd.arg = 0;
	app_cmd.flags = MMC_RSP_SHORT | MMC_RSP_CRC;

	for (i = 10; i; i--) {	//modified by hzh, 100 -> 10
		
		set_current_state(TASK_INTERRUPTIBLE);
		schedule_timeout(HZ/10);	//add by hzh, delay for card ready
		
		/* first probe for a SD card */
		err = mmc_wait_for_cmd(host, &app_cmd, 0);
		if(err == MMC_ERR_NONE) {	//add by hzh
			cmd.opcode = SD_SEND_OP_COND;
	...
	DBG("card %s\n", (cmd.resp[0]&MMC_CARD_BUSY)?"ready":"busy");	//add by hzh
...
static void mmc_discover_cards(struct mmc_host *host, int is_sd)
{
	...
	while (1) {
		...
		mmc_decode_cid(&cid, cmd.resp, is_sd);
...
void mmc_set_clock(struct mmc_host *host, int write)
{
	struct mmc_card *card;
	unsigned int max_dtr = 0xffffffff;

	list_for_each_entry(card, &host->cards, node)
		if (!mmc_card_dead(card) && max_dtr > card->csd.max_dtr)
			max_dtr = card->csd.max_dtr;

	if (write && host->card_selected) {
		max_dtr /= host->card_selected->csd.rwfactor;
	}
	
	if(max_dtr > host->f_max)
		max_dtr = host->f_max;

	if ( host->ios.clock != max_dtr ) {
...
static void mmc_setup(struct mmc_host *host)
{
	...
	if (host->ios.power_mode != MMC_POWER_ON) {
		...
		err = mmc_send_op_cond(host, 0, &ocr, &is_sd);
	 ...
	 */
	mmc_send_op_cond(host, host->ocr, NULL, &is_sd);
	mmc_discover_cards(host, is_sd);

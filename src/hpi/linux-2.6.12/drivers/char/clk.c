...
{
    upllcon = (unsigned long)ioremap(UPLLCON,0x00000004);
    clkcon  = (unsigned long)ioremap(CLKCON,0x00000004);
    clkslow = (unsigned long)ioremap(CLKSLOW,0x00000004);
    clkdivn = (unsigned long)ioremap(CLKDIVN,0x00000004);
...
*/
 /* writel(0x000059f7,intsubmsk);
	...
	writel(readl(intsubmsk)&(~(0x1<<9)),intsubmsk);
...

{
    ...
    printk(KERN_ALERT "CLKCON = %08x\nCLKSLOW=%08x\nCLKDIVN=%08x\nUPLLCON=%08x\n"
     ,(unsigned int)clkconbak,(unsigned int)clkslowbak,(unsigned int)clkdivnbak,(unsigned int)upllconbak);

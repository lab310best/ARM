...
{
  //printk(KERN_ERR "line %d passed,cirrus_send_start enter.\n",__LINE__);
	cirrus_t *priv = (cirrus_t *) dev->priv;
	u16 status;
	...
	//printk(KERN_ERR "line %d passed,(BusST)status=%d.\n",__LINE__,status);
	if ((status & TxBidErr)) {
	...
	//printk(KERN_ERR "line %d passed,valid frame size!\n",__LINE__);
	if (!(status & Rdy4TxNOW)) {
		spin_unlock_irq(&priv->lock);
...
static irqreturn_t cirrus_interrupt (int irq,void *id,struct pt_regs *regs)
{
	...
	return IRQ_RETVAL(handled);//IRQ_HANDLED;//IRQ_RETVAL(handled);
}
...
{
	int result;
	//printk(KERN_INFO "line %d passed,enter cirrus_start.",__LINE__);
	...
	if ((result = request_irq (dev->irq,&cirrus_interrupt,0,dev->name,dev)) < 0) {
		...
		printk (KERN_ERR "%s: register interrupt %d successfuly\n",dev->name,dev->irq);
...
int __init cirrus_probe (struct net_device *dev)
{
	...
	__raw_writel(0x2211d110,S3C2410_BWSCON); 
        __raw_writel(0x1f7c,S3C2410_BANKCON3);   //fla
	...
	/*if(request_mem_region(0x19000000,0x00000008,dev->name)!=NULL)    //fla
	 { printk(KERN_ERR "ok,can do ioremap,line %d passed\n",__LINE__);
        vcs8900 = ioremap_nocache(0x19000000,0x00000004);
        printk(KERN_ERR " do ioremap,vcs8900= %#p\n ",vcs8900);
    }else printk(KERN_ERR "request_mem_region err,so can't do ioremap,line %d passed",__LINE__);
	*/
	dev->base_addr = (0xe0000000+0x300);
	//printk(KERN_INFO "VCS8900=%#p",vcs8900);
	printk(KERN_INFO "dev->base_addr=%#p",dev->base_addr);
	dev->irq = CIRRUS_IRQ;
       //printk(KERN_ERR "why!\nline %d passed!\n",__LINE__);
	/* module parameters override everything */
	if (io > 0) {dev->base_addr = io;
	printk(KERN_ERR "io>0,line %d passed!\n",__LINE__);}
	if (irq > 0) {dev->irq = irq;
	printk(KERN_ERR "irq>0,line %d passed!\n",__LINE__);}
	...
	//printk(KERN_ERR "line %d passed,just set irq=%d and base_addr=%#p",__LINE__,dev->irq,dev->base_addr);
	if (!request_region (dev->base_addr,0xfff,dev->name)) {      //16 to 0xfff
		printk (KERN_ERR "%s: can't get I/O port address 0x%lx\n",dev->name,dev->base_addr);
		return -EBUSY;
	}
printk(KERN_ERR "line %d passed,request_region() OK!success get IO port address!\n",__LINE__);
#ifdef CONFIG_SA1100_FRODO
	frodo_reset (dev);
#endif	/* #ifdef CONFIG_SA1100_FRODO */

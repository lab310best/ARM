...
static int s3c2410_nand_calc_rate(int wanted, unsigned long clk, int max)
{
	...
	result = (wanted * (clk / 1000)) / 1000000;	//hzh
	result++;
				   ...
				   struct s3c2410_nand_set *set)
{
	...
	if (info->is_s3c2440) {
		//=== hzh
		__u32 cur = readl(info->regs + S3C2440_NFCONT) | S3C2440_NFCONT_ENABLE;
	...
	if (hardware_ecc) {
	  ...
	  chip->eccmode	    = NAND_ECC_NONE;//NAND_ECC_NONE;//SOFT BEFORE
	}
#if 0
	//=== hzh
{
	__u32 chip_id;
	printk("NF_CONF is 0x%lx\n", readl(info->regs + S3C2410_NFCONF));
	chip->select_chip(&nmtd->mtd, 0);
	chip->hwcontrol(&nmtd->mtd, NAND_CTL_SETCLE);
	printk("NF_CONT is 0x%lx\n", readl(info->regs + S3C2440_NFCONT));
	chip->hwcontrol(&nmtd->mtd, NAND_CTL_SETNCE);
	writeb(0x90, chip->IO_ADDR_W);
	chip->hwcontrol(&nmtd->mtd, NAND_CTL_CLRCLE);
	chip->hwcontrol(&nmtd->mtd, NAND_CTL_SETALE);
	writeb(0x00, chip->IO_ADDR_W);
	chip->hwcontrol(&nmtd->mtd, NAND_CTL_CLRALE);
	udelay(10);
	chip_id  = readb(chip->IO_ADDR_R) << 8;
	chip_id |= readb(chip->IO_ADDR_R);
	chip->hwcontrol(&nmtd->mtd, NAND_CTL_CLRNCE);
	chip->select_chip(&nmtd->mtd, -1);
	printk("NF_CONT is 0x%lx\n", readl(info->regs + S3C2440_NFCONT));
	printk("NAND chip ID is 0x%x\n", chip_id);
}
	//===
#endif
}
